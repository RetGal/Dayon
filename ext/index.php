...
function updateAssisted($pdo, $token, $open, $address, $rport, $localAddress, $ice) {
    // always empty for the time being
    if (empty($ice)) {
        $sql = "UPDATE tokens SET assisted = :address, rport = :rport, assisted_local = :laddr, open = CASE WHEN ((:open != -1) OR (:open = -1 AND (open = -1 OR open IS NULL))) THEN :open ELSE open END, ts = :ts WHERE token = :token";
    } else {
        $sql = "UPDATE tokens SET assisted = :address, rport = :rport, assisted_local = :laddr, open = CASE WHEN ((:open != -1) OR (:open = -1 AND (open = -1 OR open IS NULL))) THEN :open ELSE open END, ts = :ts, assisted_ice = :assisted_ice WHERE token = :token";
    }
    $ts = time();
    // assisted -1 unknown, 0 not open, 1 open -> ts
    // prevent updates of open from a known to an unknown state
    $open = $open == 1 ? $ts : $open;
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':address', $address, PDO::PARAM_STR);
