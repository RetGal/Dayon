...
public class XZ_Zipper implements Zipper {
    ...
    public MemByteBuffer zip(MemByteBuffer unCompressed) throws IOException {
		final MemByteBuffer compressed = new MemByteBuffer();
		final XZOutputStream xzOutputStream = new XZOutputStream(compressed, new LZMA2Options(6));
    ...
    public MemByteBuffer unzip(MemByteBuffer compressed) throws IOException {
		final MemByteBuffer unCompressed = new MemByteBuffer();
		final XZInputStream xzInputStream = new XZInputStream(new ByteArrayInputStream(compressed.getInternal(), 0, compressed.size()));
