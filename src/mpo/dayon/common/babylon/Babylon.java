...
public abstract class Babylon
{
    ...
    /**
     * Attempt to format the tag value; if the actual tag value is missing or the tag value
     * could not be formatted for whatever reason, then the <code>toString</code> of the argument
     * array is appended to the tag value...
     */
    private static String formatValue(Locale locale, String tagValue, String tag, Object... arguments)
    {
        ...
        if (tagValue != tag) // The identity equality is fine; that's what I want!
        {
            ...
            catch (IllegalFormatException ex)
            {
                ...
                formattedTagValue = tagValue + " " + Arrays.toString(arguments); // what else can I do here?
            }
        }
        else
        {
