...
public class Capture
{
     ...
     * <p/>
     * That's the actual payload size of a capture to paint it on the screen - that's the only amount of data
     * I would need to send to the assistant over the network. But I've to send some extra-info for the location,
     * size, etc... as well as some un-marshalling extra-stuff (e.g., len). Then, I'm going to encode and compress
     * all that data and I want to compute on the assistant side the actual compression ratio compared to that
     * initial amount of byte.
     * <p/>
    ...
    {
        // The only way the tile 'length' may change is when the capture engine has been re-configured.
        // In that case (for the sake of simplicity) a FULL capture will be sent.
