...
public abstract class UnitUtilities {
	public enum BitUnit {
		Kbit("Kbit", "kilo", Math.pow(10, 3)), Mbit("Mbit", "mega", Math.pow(10, 6)), Gbit("Gbit", "giga", Math.pow(10, 9)), Tbit("Tbit", "tera",
				Math.pow(10, 12)), Pbit("Pbit", "peta", Math.pow(10, 15)), Ebit("Ebit", "exa",
						Math.pow(10, 18)), Zbit("Zbit", "zetta", Math.pow(10, 21)), Ybit("Ybit", "yotta", Math.pow(10, 24));
	...
	public static String toBitSize(double bits) {
		...
		for (int idx = units.length - 1; idx >= 0; idx--) {
			...
			if (bits >= unit.value) {
				return String.format("%.2f %s", bits / unit.value, unit.symbol);
			}
		}

		return String.format("%.2f %s", bits, "bit");
	}

	public enum ByteUnit {
		K("K", "kilo", Math.pow(2, 10)), M("M", "mega", Math.pow(2, 20)), G("G", "giga", Math.pow(2, 30)), T("T", "tera", Math.pow(2, 40)), P("P", "peta",
				Math.pow(2, 50)), E("E", "exa", Math.pow(2, 60)), Z("Z", "zetta", Math.pow(2, 70)), Y("Y", "yotta", Math.pow(2, 80));
	...
	public static String toByteSize(double bytes, boolean withDecimal) {
		...
		for (int idx = units.length - 1; idx >= 0; idx--) {
			...
			if (bytes >= unit.value) {
				if (withDecimal) {
					return String.format("%.2f %s", bytes / unit.value, unit.symbol);
				}
		...
		if (withDecimal) {
			return String.format("%.2f %s", bytes, "");
		}
