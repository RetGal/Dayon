...
import mpo.dayon.common.squeeze.*;
                   ...
                   CaptureEngineListener
{
    ...
    public void start(int queueSize)
    {
        // THREAD = 1
        //
        //  The parallel processing is within the compressor itself - here we want
        //  to ensure a certain order of processing - if need more than one thread
        ...
        //
        //  - queue full because we could not compress the last capture when a new
        //    one is available => too many captures (!)
        //
        //  - we could not send the last compressed capture over the network as the
     ...
     * Must not block.
     * <p/>
     * Each capture is posted in the right order (according to its capture-id) from a SINGLE thread. Have a look
     * to the rejection execution handler => between the poll() and re-execute() another thread must not execute
     * during the rejection processing to keep the right order of the captures in the queue.
     * <p/>
     * Note that the current implementation should never block and never throw any rejected exception.
     */
        ...
        protected void execute() throws Exception
        {
            try
            {
                ...
                synchronized (reconfigurationLOCK)
                {
                    ...
                    if (reconfigured)
                    {
                        cache = xconfiguration.useCache()
                                ? new RegularTileCache(xconfiguration.getCacheMaxSize(), xconfiguration.getCachePurgeSize())
                                : new NullTileCache();
    ...
    private void fireOnCompressed(Capture capture,
