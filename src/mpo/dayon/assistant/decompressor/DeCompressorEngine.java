                   ...
                   NetworkCaptureMessageHandler
{
    ...
    public void start(int queueSize)
    {
        // THREAD = 1
        //
        //  The parallel processing is within the de-compressor itself - here we want
        //  to ensure a certain order of processing - if need more than one thread then
        ...
        // Rejection Policy
        //
        // Blocking pattern when queue full; that means we're not decompressing fast enough; when our queue is full
        // then the network receiving thread is going to stop reading from the assisted side which in turn is going
        // to slow down sending its capture leaving us some time to catch up.
        //
        // Having our queue full is quite unlikely; I would say the network will limit the number of capture/tiles
        // being sent and I guess that decompressing is much faster then compressing (unless our PC is quite weak
        // compared to the assisted one; let's not forget the JAVA capture is awful regarding the performance as
    ...
    private class MyExecutable extends Executable
    {
        ...
        protected void execute() throws Exception
        {
            try
            {
                ...
                final double ratio = capture.computeCompressionRatio(1/*magic-number*/ + message.getWireSize());
